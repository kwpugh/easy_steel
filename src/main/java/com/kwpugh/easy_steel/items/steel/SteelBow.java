...
public class SteelBow extends BowItem
{
         ...
         if (!itemstack.isEmpty() || flag)
         {
            ...
            if (!((double)f < 0.1D))
            {
               ...
               if (!worldIn.isRemote)
               {
                  ...
                  abstractarrowentity.setDirectionAndMovement(playerentity, playerentity.rotationPitch, playerentity.rotationYaw, 0.0F, f * 4.5F, 1.0F);
                  if (f == 1.0F)
                  {
   ...
   public boolean getIsRepairable(ItemStack toRepair, ItemStack repair)
   {
	...
	public void addInformation(ItemStack stack, @Nullable World worldIn, List<ITextComponent> tooltip, ITooltipFlag flagIn)
	{
		...
		tooltip.add((new TranslationTextComponent("item.easy_steel.bow.line1").mergeStyle(TextFormatting.GREEN)));
	}
}