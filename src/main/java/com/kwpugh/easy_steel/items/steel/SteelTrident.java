...
public class SteelTrident extends Item
{
	   public SteelTrident(Item.Properties builder) {
	      super(builder);
	         ...
	         if (i >= 10) {
	            ...
	            if (j <= 0 || playerentity.isWet()) {
	               if (!worldIn.isRemote) {
	                  ...
	                  if (j == 0) {
	                     SteelTridentEntity steeltridententity = new SteelTridentEntity(worldIn, playerentity, stack);
	                     steeltridententity.shoot(playerentity, playerentity.rotationPitch, playerentity.rotationYaw, 0.0F, 2.5F + (float)j * 0.5F, 1.0F);
	                     if (playerentity.abilities.isCreativeMode) {
	                    	 steeltridententity.pickupStatus = AbstractArrowEntity.PickupStatus.CREATIVE_ONLY;
	                     }

	                     worldIn.addEntity(steeltridententity);
	                     worldIn.playMovingSound((PlayerEntity)null, steeltridententity, SoundEvents.ITEM_TRIDENT_THROW, SoundCategory.PLAYERS, 1.0F, 1.0F);
	                     if (!playerentity.abilities.isCreativeMode) {
